...
def rock_superstars
  ...
  execute(<<-SQL)
    SELECT
      albums.artist, COUNT(DISTINCT albums.asin) AS cnt
    FROM
      albums
    INNER JOIN
...
def expensive_tastes
  ...
  execute(<<-SQL)
    SELECT
      styles.style, (SUM(album_values.price) / SUM(album_values.cnt)) as avg_value
    FROM (
      SELECT
        albums.asin AS asin, albums.price AS price, COUNT(*) AS cnt
      FROM
        albums
      INNER JOIN
        tracks ON albums.asin = tracks.album
    ...
    INNER JOIN
      styles ON styles.album = album_values.asin
    GROUP BY
      styles.style
    ORDER BY
      avg_value DESC
